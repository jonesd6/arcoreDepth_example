...
import com.matt.arcore.java.sharedcamera_example.TOF_ImageReader;
...
public class BackgroundRenderer {
  ...
  private TOF_ImageReader tof_imageReader;
  ...
  **/
  ...
  public void draw(Frame frame, int vizMode, int depth_thresh) {
    ...
    GLES32.glTexImage2D(GLES32.GL_TEXTURE_2D,0,GLES32.GL_R16UI,parentActivity.tof_imageReader.WIDTH,
            parentActivity.tof_imageReader.HEIGHT,0,GLES32.GL_RED_INTEGER,
            GLES32.GL_UNSIGNED_SHORT, parentActivity.tof_imageReader.depth16_raw);
    ShaderUtil.checkGLError(TAG, "after upload");

    Log.v(TAG, "depth width: " + tof_imageReader.WIDTH + " height: " + tof_imageReader.HEIGHT);
    // ***************************************
    ...
    float new_width = parentActivity.tof_imageReader.WIDTH;
    float new_height = landscape_aspect * new_width;
    float y_offset = (parentActivity.tof_imageReader.HEIGHT - new_height) / 2.0f;
